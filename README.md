...
├── figures/                        — 13 source PDF vector figures (generated by scripts, unchanged)
├── figures_eps/                    — 13 EPS versions (converted from PDF for MMTB submission)
│   ├── fig_liquidus.eps
│   ├── fig_scm_conversion.eps
│   ├── fig_scm_time_vs_size.eps
│   ├── fig_scm_sensitivity.eps
│   ├── fig_E_pO2.eps
│   ├── fig_overpotential_bv.eps
│   ├── fig_voltage_split.eps
│   ├── fig_energy_sensitivity.eps
│   ├── fig_cl2_boundary.eps
│   ├── fig_monte_carlo.eps
│   ├── fig_sensitivity_tornado.eps
│   ├── fig_overpotential_window.eps      — verification-only (not referenced)
│   └── fig_heat_balance_revised.eps      — verification-only (not referenced)
...
The manuscript references EPS figures in `figures_eps/` (converted from PDF originals for MMTB submission). Compilation with `pdflatex` + `epstopdf` automatically converts them to PDF on-the-fly.
